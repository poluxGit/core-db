...
VMIN=$(printf "%02d\n" $3)
DATEVAL=`date +%Y%m%d`
RELEASES_DIR=./releases/${DATEVAL}_$SCHEMA-R-$VMAJ.$VMIN
SQL_OUTFILE=$RELEASES_DIR/DB-SCRIPTS_$SCHEMA-R-$VMAJ.$VMIN.sql
SQL_FINAL=$RELEASES_DIR/DB-SCRIPTS_$SCHEMA-R-$VMAJ.$VMIN-final.sql
SCRIPTS_DIR=./wip
...
# Par répertoire
for lfile in `find $SCRIPTS_DIR -name *.sql`
do
...
echo "- Final Step - Inject Parameterized values."
